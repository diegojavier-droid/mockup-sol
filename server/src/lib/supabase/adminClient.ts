 ...
 *
 * Uses SUPABASE_SERVICE_ROLE_KEY. This key BYPASSES Row-Level Security.
 *
 * Import rules:
 *   ✅ Allowed:
 ...
 * frontend code. Never expose SUPABASE_SERVICE_ROLE_KEY via publicEnv.ts,
 ...
 *
 * Do NOT cache across requests with per-user state — the service role key
 * has no user identity. If per-user behavior is required, use the anon
 * client and pass the user's JWT explicitly.
 */

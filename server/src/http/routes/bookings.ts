...
const createSchema = z.object({
  /** «Color y corte» en un solo turno. La forma singular sigue viva. */
...
export function createBookingsRoute(env: ServerEnv) {
  ...
  route.post("/", async (c) => {
    ...
    let parts: ReturnType<typeof normalizeServiceParts>;
    let loaded: Awaited<ReturnType<typeof loadServiceParts>>;
    try {
    ...
    const context = loaded?.contexts[0] ?? null;
    const serviceLabel = parts.map((p) => p.serviceSlug).join("+");
    /**
    ...
    if (!context || !loaded) throw reject("unknown_service");
    ...
    // Se arma por prestación y no desde la cotización compuesta: cada
    // ítem principal tiene que llevar SU personalización, no la del
    // primer servicio.
    const items: CreateBookingItem[] = partQuotes.flatMap((partQuote, i) =>

...
select pg_temp.chequear('A2 · trae el nombre de la clienta, no un id',
  (select clienta = 'Marta Gómez' from public.pending_invoices() limit 1));

select pg_temp.chequear('A3 · trae qué se hizo, para tipearlo en ARCA',
  (select servicios = 'Corte y color' from public.pending_invoices() limit 1));

select pg_temp.chequear('A4 · trae el importe cobrado',
  (select cobrado > 0 from public.pending_invoices() limit 1));
...
select pg_temp.chequear('B5 · queda en el registro de cambios, con lo cobrado al lado',
  (select (detail->>'importe')::int = 20000 and detail ? 'cobrado'
...
select pg_temp.chequear('F1 · suma lo facturado del año',
  (public.invoicing_summary()->>'facturado')::bigint = 20000);

select pg_temp.chequear('F2 · dice cuánto falta facturar',
  (public.invoicing_summary()->>'pendiente')::bigint = 40000 + 60000);

select pg_temp.chequear('F3 · y cuántas atenciones son',
  (public.invoicing_summary()->>'cuantos_pendientes')::int = 2);
